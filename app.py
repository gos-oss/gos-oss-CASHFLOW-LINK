...
# =============================================================================
# 2. FUNCIONES DE LIMPIEZA ROBUSTAS
# =============================================================================
def limpiar_valor_moneda(val):
    """Convierte texto de moneda a número flotante para realizar cálculos matemáticos reales."""
    ...
    # Manejar formatos contables que usan paréntesis para números negativos: (64.823.185)
...
def formato_moneda_texto(x):
    """Aplica el signo $ a los números para visualización impecable en tablas."""
...
def pintar_negativos(val):
    """Pinta de rojo fuerte los valores negativos en la matriz."""
...
# =============================================================================
# 4. PANTALLA PRINCIPAL Y PROCESAMIENTO
# =============================================================================
...
if uploaded_file is not None and hoja_seleccionada is not None:
    try:
        ...
        # 1. Renombrar la primera columna a "Concepto" y hacer limpieza extrema de espacios
        df_raw.rename(columns={df_raw.columns[0]: "Concepto"}, inplace=True)
        col_concepto = "Concepto"
        # La función .str.strip() es clave aquí para eliminar espacios invisibles al final de las palabras
        df_raw[col_concepto] = df_raw[col_concepto].fillna("").astype(str).str.strip().replace(['nan', 'None', 'NaN'], '')
        ...
        # 3. CONVERSIÓN A NÚMEROS REALES
        df_procesado = df_raw[[col_concepto] + cols_fechas].copy()
        for col in cols_fechas:
            df_procesado[col] = df_procesado[col].apply(limpiar_valor_moneda)

        # --- EXTRACCIÓN DE FILAS CLAVE (BÚSQUEDA FLEXIBLE) ---
        # Se quitan los anclajes estrictos (^ $) para que coincida aunque el texto tenga variaciones mínimas
        row_saldo_acum = df_procesado[df_procesado[col_concepto].str.contains("Saldo acumulado", case=False, na=False)]
        row_posicion_dia = df_procesado[df_procesado[col_concepto].str.contains("Posicion del dia", case=False, na=False)]
        row_saldo_ini = df_procesado[df_procesado[col_concepto].str.contains("Saldo inicial", case=False, na=False)]
            ...
            for col_fecha in cols_fechas:
                ...
                if val_saldo < 0:
                    try:
                        ...
                        # La resta de días es exacta gracias a los objetos date
        ...
        idx_ingresos = df_procesado.index[df_procesado[col_concepto].str.contains("Total ingresos", case=False, na=False)].tolist()
        idx_egresos = df_procesado.index[df_procesado[col_concepto].str.contains("Total Egresos", case=False, na=False)].tolist()
